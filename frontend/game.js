...
// Method 2: Use image Detection (backup)
...
// Get file extension (based on WebP Support status + Layout Configuration forcePng)
function getExt(pngFile) {
  // star-working-spritesheet.png Too wide,WebP Unsupported, always use PNG
  if (pngFile === 'star-working-spritesheet.png') {
    return '.png';
  }
  // If forced in layout config PNG, just use .png
...
async function loadMemo() {
  ...
  try {
    ...
    console.error('Loading memo Failure:', e);
...
function updateLoadingProgress() {
  ...
  if (loadingText) {
    loadingText.textContent = `Loading Star Pixel Office... ${percent}%`;
  }
}
...
const STATES = {
  ...
  error: { name: 'An error occurred', area: 'error' }
};

const BUBBLE_TEXTS = {
  idle: [
    'On Standby: Ears are perked up',
    "I'm here, ready to start anytime",
    'Clean up the desktop first',
    'Phew——Give the brain a break',
    'Be elegant and efficient today',
    'Waiting for a More Accurate Strike',
    "Coffee's Still Hot, Inspiration's Still Here",
    'I added for you in the background Buff',
    'Status: Focused / Charging',
    "Kitty says: It's okay to slow down"
  ],
  writing: [
    'Enter focus mode: Do not disturb',
    'First, run through the critical path',
    'I simplify complexity',
    'Put bug Lock in the cage',
    'Halfway through writing, save first',
  ...
  researching: [
    "I'm digging for evidence chain",
    'Let me boil down the information into conclusions',
    'Found it: The key is here',
    'First, control the variables',
    "I'm investigating: why is it like this",
    'Write intuition into validation',
    'Locate first, then optimize',
    "Don't rush, draw a cause-and-effect diagram first"
  ],
  executing: [
    "Executing: Don't blink",
    'Break tasks into small pieces and tackle them one by one',
    'Start running pipeline',
    'One-click advance: Go',
    'Let the results speak for themselves',
    'Start with the minimum viable, then make it beautiful'
  ],
  syncing: [
    'Syncing: Lock today into the cloud',
    "Backup is not a ritual, it's a sense of security",
    "Writing…Don't power off",
    'Hand over changes to timestamp',
    'Cloud sync: click',
    "Don't move until sync is complete",
    'Save your future self from disaster',
    'One More Backup, One Less Regret'
  ],
  error: [
    "Alarm Triggered: Don't Panic",
    'I smell bug The taste of',
    'Reproduce first, then discuss fixing',
    ...
    "I'm on it: locating root cause immediately",
    "Don't Worry, I've Seen This Many Times",
    'Alert: Let the issue reveal itself'
  ],
  cat: [
    'Meow~',
    'Glug glug…',
    'Wag the Tail',
    'Sunbathing is the happiest',
    "Someone's here to see me!",
    "I'm the mascot of this office",
    'Stretch',
    "Is today's can ready?",
    'Purring',
    'This position has the best view'
  ]
};
...
// breakroom / writing / error Of the area agent Distribution Location (multi agent Staggered
...
// Initialize: detect first WebP Support, then restart the game
...
function preload() {
  ...
  // From LAYOUT Read total resource count (avoid magic number)
...
function create() {
  ...
  const plaqueText = game.add.text(plaqueX, plaqueY, "Star's agent office", {
  ...
  // === New desk (from LAYOUT, force transparency PNG)===
  ...
  // === Star Working at the Desk (from LAYOUT)===
  this.anims.create({
    key: 'star_working',
  ...
  loadMemo();
  fetchStatus();
  // Force add a test Nika agent Render
  const testNika = {
    agentId: 'agent_nika',
    name: 'Nika',
    isMain: false,
  ...
  // For testing: let Nika simulate walking around
...
function showCatBubble() {
  ...
  const texts = BUBBLE_TEXTS.cat || ['Meow~', 'Glug glug…'];
  const text = texts[Math.floor(Math.random() * texts.length)];
...
function fetchAgents() {
  fetch('/agents?t=' + Date.now(), { cache: 'no-store' })
    ...
    .catch(error => {
      console.error('Pull agents Failure:', error);
    });
}
...
function renderAgent(agent) {
  ...
  // Get this agent Position in the area
  ...
  // Transparency (offline/Pending Approval/Turn semi-transparent when rejected)
  let alpha = 1;
  ...
  if (!agents[agentId]) {
    ...
    container.setDepth(1200 + (isMain ? 100 : 0)); // Put at the top!

    // Pixel character: use star icon for clarity
